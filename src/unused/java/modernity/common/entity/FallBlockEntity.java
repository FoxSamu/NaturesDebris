        ...
        if( fallTile.isAir( world, new BlockPos( this ) ) ) {
            ...
            if( ! this.world.isRemote ) {
                ...
                if( isConcretePowder && speedSq > 1.0D ) {
                    BlockRayTraceResult rtr = world.rayTraceBlocks( new RayTraceContext( new Vec3d( prevPosX, prevPosY, prevPosZ ), new Vec3d( posX, posY, posZ ), RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.SOURCE_ONLY, this ) );
                ...
                if( ! onGround && ! floatConcrete ) {
                    ...
                    if( world.isAirBlock( new BlockPos( posX, posY - 0.01, posZ ) ) ) {
                        if( ! floatConcrete && FallBlock.canFallThrough( world.getBlockState( new BlockPos( posX, posY - 0.01, posZ ) ) ) ) {
                            this.onGround = false;
                            return;
                        }
                    }
