...
public class RainPrecipitation implements IPrecipitation {
    @OnlyIn( Dist.CLIENT )
    @Override
    ...
    public void playSound( double x, double y, double z, boolean above, World world, float strength ) {
        if( above ) {
            world.playSound( x, y, z, SoundEvents.WEATHER_RAIN_ABOVE, SoundCategory.WEATHER, 0.1F * strength, 0.5F, false );
        } else {
            world.playSound( x, y, z, SoundEvents.WEATHER_RAIN, SoundCategory.WEATHER, 0.2F * strength, 1, false );
        }
    }
    ...
    public void blockUpdate( World world, BlockPos pos ) {
        ...
        if( doesPuddleGenerate( world, heightPos ) ) {
            ...
            if( state.isAir( world, heightPos ) ) {
                ...
                MDBlocks.PUDDLE.rainTick( world, pos, state, 0.2 );
            }
        }
    }
