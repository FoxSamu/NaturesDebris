...
public final class MDLayerUtil {
    ...
    public static <T extends IArea, C extends IExtendedNoiseRandom<T>> ImmutableList<IAreaFactory<T>> buildSurfaceProcedure( LongFunction<C> contextFactory ) {
        IAreaFactory<T> factory = new SurfaceBiomeFactory<>( EMDDimension.SURFACE, contextFactory.apply( 5291L ) );
        ...
        IAreaFactory<T> rivers = RiverInitLayer.INSTANCE.apply( contextFactory.apply( 5102L ) );
        rivers = repeat( 1000L, ZoomLayer.NORMAL, rivers, 6, contextFactory );
        rivers = RiverLayer.INSTANCE.apply( contextFactory.apply( 9911L ), rivers );
        rivers = SmoothLayer.INSTANCE.apply( contextFactory.apply( 7716L ), rivers );

        factory = RiverMixLayer.INSTANCE.apply( contextFactory.apply( 1234L ), factory, rivers );
