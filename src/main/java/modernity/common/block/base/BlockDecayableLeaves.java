...
public class BlockDecayableLeaves extends BlockBase implements IShearable {
    ...
    public BlockDecayableLeaves( String id, Tag<Block> logTag, Properties properties, Item.Properties itemProps ) {
        super( id, properties, itemProps );
        this.logTag = logTag;
        setDefaultState( stateContainer.getBaseState().with( DISTANCE, 7 ).with( PERSISTENT, false ) );
    }

    public BlockDecayableLeaves( String id, Tag<Block> logTag, Properties properties ) {
        super( id, properties );
        this.logTag = logTag;
        setDefaultState( stateContainer.getBaseState().with( DISTANCE, 7 ).with( PERSISTENT, false ) );
    }
    ...
    private int getDistance( IBlockState neighbor ) {
        if( logTag.contains( neighbor.getBlock() ) ) {
            ...
            return neighbor.getBlock() instanceof BlockDecayableLeaves ? neighbor.get( DISTANCE ) : 7;
        }
    }
