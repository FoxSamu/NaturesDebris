...
public class MinableFeature extends Feature<MinableFeature.Config> {
    ...
    protected boolean generate( IWorld world, Random random, Config config, double x1, double x2, double z1, double z2, double y1, double y2, int minX, int minY, int minZ, int xzSize, int ySize ) {
        int amountPlaced = 0;
                                    ...
                                    if( dx * dx + dy * dy + dz * dz < 1 ) {
                                        ...
                                        if( ! placedBlocks.get( idx ) ) {
                                            ...
                                            if( config.target.test( world.getBlockState( mpos ) ) ) {
                                                world.setBlockState( mpos, config.state, BlockUpdates.NOTIFY_CLIENTS );
                                                amountPlaced++;
                                            }
                                        }
                                    }
