...
package modernity.api.util.math;
...
public interface ILongScrambler {
     ...
     */
    static ILongScrambler lgc(long add, long mul) {
        return seed -> seed * mul + add;
    }
    static ILongScrambler xorshift(int... leftShifts) {
    ...
    static ILongScrambler xorshift(long... seeds) {
     ...
     */
    static ILongScrambler hash(int shift, long lgcAdd, long... seeds) {
        if(seeds == null || seeds.length == 0) throw new IllegalArgumentException();
    ...
    default ILongScrambler masked(long mask) {
