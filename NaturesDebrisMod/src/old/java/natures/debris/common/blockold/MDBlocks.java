...
import natures.debris.common.blockold.base.ICustomBlockItem;
import natures.debris.common.blockold.loot.IBlockDrops;
import natures.debris.common.blockold.loot.MDBlockDrops;
import natures.debris.common.recipes.data.RecipeDataTypes;
...
public final class MDBlocks {
    ...
    private static final RegistryHandler<Block> BLOCKS = new RegistryHandler<>("natures/debris");
    private static final RegistryHandler<Item> ITEMS = new RegistryHandler<>("natures/debris");
    ...
    public static Supplier<BlockState> supply(String id) {
        return new BlockStateSupplier(new ResourceLocation("natures/debris", id));
    }

    public static Supplier<IItemProvider> supplyI(String id) {
        return Lazy.of(() -> ForgeRegistries.ITEMS.getValue(new ResourceLocation("natures/debris", id)));
    }
    ...
    public static class BlockConfig<T extends Block> {
        ...
        public T create() {
            ...
            if (drops != null) {
                IModernityOld.get().getDataService().addBlockDrops(block, drops);
            }
            for (IRecipeData recipe : recipes) {
                IModernityOld.get().getDataService().addRecipe(recipe);
            }
