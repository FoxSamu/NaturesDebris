...
import modernity.common.blockold.tree.DecayLeavesBlock;
...
import static modernity.generic.util.BlockUpdates.*;
...
public class SphericalTree extends Tree {
    ...
    protected void generateTree(Consumer<BlockPos> logs, IWorld world, Random rand, BlockPos pos) {
        ...
        for(Direction facing : Direction.Plane.HORIZONTAL) {
            ...
            for(int i = 0; i < len; i++) {
                rpos.move(facing);
                world.setBlockState(rpos, log(axis), NOTIFY_CLIENTS | NO_NEIGHBOR_REACTIONS | NO_RENDER);
                logs.accept(rpos.toImmutable());
            }
        }
