...
public class DoorBlock extends Block {
    ...
    private DoorHingeSide getHingeSide(BlockItemUseContext ctx) {
        ...
        int hingeWeight = (
            rightState.isCollisionShapeOpaque(world, rightPos) ? -1 : 0
        ) + (
            rightUpState.isCollisionShapeOpaque(world, rightUpPos) ? -1 : 0
        ) + (
            leftState.isCollisionShapeOpaque(world, leftPos) ? 1 : 0
        ) + (
            leftUpState.isCollisionShapeOpaque(world, leftUpPos) ? 1 : 0
        );
    ...
    public boolean isValidPosition(BlockState state, IWorldReader worldIn, BlockPos pos) {
        ...
        if(state.get(HALF) == DoubleBlockHalf.LOWER) {
            return stateBelow.isSolidSide(worldIn, posBelow, Direction.UP);
        } else {
